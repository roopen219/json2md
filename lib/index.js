...
function json2md(data, prefix, _type, noNewLine) {
  prefix = prefix || ""
  ...
  if (Array.isArray(data)) {
    for (let i = 0; i < data.length; ++i) {
      content.push(indento(json2md(data[i], "", _type, noNewLine), 1, prefix))
    }
    return content.join("\n")
  } else if (_type) {
    let mdText = "";
    ...
    if (typeof func === "function") {
      mdText += indento(func(_type ? data : data[type], json2md), 1, prefix) + (noNewLine ? '' : "\n");
    } else {
